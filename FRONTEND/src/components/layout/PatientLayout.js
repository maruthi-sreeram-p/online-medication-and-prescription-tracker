...
import {
  ...
  Settings, Menu, Logout, LocalHospital, PersonSearch
...
const PatientLayout = () => {
  ...
  // Read real name from localStorage (set during login)
  ...
  const menuItems = [
    ...
    { text: 'Find Doctors', icon: <PersonSearch />, path: '/patient/find-doctors' },
  ...
  const drawer = (
    <Box sx={{ height: '100%', bgcolor: '#ffffff', borderRight: '1px solid #e2e8f0' }}>
      ...
      {/* Patient Profile Card — dynamic from localStorage */}
        ...
        {menuItems.map((item) => (
          <ListItem key={item.text} disablePadding sx={{ mb: 1 }}>
            ...
            >
              ...
              />
              {item.text === 'Find Doctors' && (
                <Box sx={{ bgcolor: '#0062ff', color: 'white', borderRadius: 1, px: 1, fontSize: '0.65rem', fontWeight: 700 }}>
                  NEW
                </Box>
              )}
            </ListItemButton>
